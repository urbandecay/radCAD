"""Operator entry point for the selected-mesh Move tool."""
...
class VIEW3D_OT_radcad_move(bpy.types.Operator):
    bl_idname = "view3d.radcad_move"
    bl_label = "Move"
    bl_description = (
        "Move selected mesh geometry with axis, edge-direction, and snap constraints"
    )
    ...
    def poll(cls, context):
        ...
        if not available:
            cls.poll_message_set("Enter Mesh Edit Mode to use Move")
        return available

    def invoke(self, context, event):
        if not _has_selected_geometry(context):
            self.report({"WARNING"}, "Select mesh geometry before using Move")
            return {"CANCELLED"}
        state["tool_mode"] = "MOVE"
