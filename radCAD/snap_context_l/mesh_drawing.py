...
def get_mesh_loosevert_array(me, edges):
    ...
    verts = np.arange(len(me.vertices), dtype="i4")
...
def gpu_index_array(values):
    """Blender 5.1 requires every GPU index to be a contiguous 4-byte integer."""
    import numpy as np

    return np.ascontiguousarray(values, dtype="i4")
...
class GPU_Indices_Mesh:
    ...
    def __init__(self, depsgraph, obj, draw_tris, draw_edges, draw_verts):
        ...
        if update:
            ...
            if len(self.tri_verts) > 0:
                self.tri_verts = gpu_index_array(self.tri_verts)
            ...
            if len(self.edge_verts) > 0:
                self.edge_verts = gpu_index_array(self.edge_verts)
            ...
            if len(self.looseverts) > 0:
                self.looseverts = gpu_index_array(self.looseverts)
    ...
    def free(self):
        ...
        if len(self.users) == 0:
            ...
            del self.edge_verts
            del self.looseverts
            GPU_Indices_Mesh._cache.pop(self.ob_data, None)
