...
def get_axis_aligned_color(vec, default_col):
    ...
    if abs(v_norm.dot(Vector((1, 0, 0)))) > 0.999999: return (0.85, 0.00, 0.00, 1.00)
    if abs(v_norm.dot(Vector((0, 1, 0)))) > 0.999999: return (0.00, 0.60, 0.00, 1.00)
    if abs(v_norm.dot(Vector((0, 0, 1)))) > 0.999999: return (0.149, 0.376, 1.0, 1.0)
    return default_col
...
def get_render_settings(ctx):
    ...
    if addon_prefs:
        ...
        prefs["COMPASS_SIZE"] = addon_prefs.compass_size
    return prefs
...
def setup_polyline_shader(sh, color, width, settings):
    ...
    try:
        ...
        sh.uniform_float("lineWidth", max(1.0, width * settings["UI_SCALE"]))
...
def draw_line(ctx, shaders, p1, p2, color, settings):
    ...
    final_color = color
    # --- TEAL KILLER ---
    if len(color) >= 3 and color[0] < 0.1 and color[1] > 0.9 and color[2] > 0.9: final_color = (1.0, 0.8, 0.0, 1.0)
...
def draw_preview_tan_tan_tan(ctx, shaders, prefs):
    ...
    vis_pts = state.get("visual_pts", [])   
    overlays = state.get("tan_input_overlays", [])
    
    orange = (1.0, 0.5, 0.0, 1.0)
    for o_pts in overlays:
        draw_polyline(ctx, shaders, o_pts, orange, prefs, custom_lift=prefs["LIFT_ARC"] - 1.0)
        
    if vis_pts:
        green = (0.0, 1.0, 0.0, 1.0)
        draw_polyline(ctx, shaders, vis_pts, green, prefs, custom_lift=prefs["LIFT_ARC"])
        
    if pts:
        black = (0.0, 0.0, 0.0, 1.0)
        draw_polyline(ctx, shaders, pts, black, prefs, custom_lift=prefs["LIFT_ARC"] + 2.0, custom_width=1.0)
        draw_points(ctx, shaders, pts, black, 4, prefs)
        
    if state.get("pivot"):
        draw_points(ctx, shaders, [state["pivot"]], (1,1,1,1), 4, prefs)

def draw_preview_tan_tan(ctx, shaders, prefs):
    pt_size = 6
    pts = state.get("preview_pts", []) 
    visual_pts = state.get("visual_pts", [])
    
    orange_col = (1.0, 0.5, 0.0, 1.0)
    black_col = (0.0, 0.0, 0.0, 1.0)
    white_col = (1.0, 1.0, 1.0, 1.0)
    
    # NO RAIL DRAWING HERE
    
    # Radius Lines (White)
    v_tan_line = state.get("viz_tangent_line")
    if v_tan_line and len(v_tan_line) == 2:
         draw_polyline(ctx, shaders, v_tan_line, white_col, prefs, custom_lift=prefs["LIFT_ARC"] + 1.0)
         
    v_dia_line = state.get("viz_diameter_line")
    if v_dia_line and len(v_dia_line) == 2:
         draw_polyline(ctx, shaders, v_dia_line, white_col, prefs, custom_lift=prefs["LIFT_ARC"] + 1.0)

    v_opp_dot = state.get("viz_opposite_dot")
    if v_opp_dot:
         draw_points(ctx, shaders, v_opp_dot, white_col, 8, prefs)
    
    if state.get("stage", 0) == 0:
        target = state.get("snap_point") if state.get("snap_point") else state.get("current")
        if target: draw_points(ctx, shaders, [target], (1, 1, 0, 1), pt_size, prefs)
    
    if state.get("pivot"): draw_points(ctx, shaders, [state["pivot"]], (1, 0, 0, 1), pt_size, prefs)
        
    if state.get("current"): draw_points(ctx, shaders, [state["current"]], (1, 1, 0, 1), pt_size, prefs)
    
    if pts:
        draw_polyline(ctx, shaders, pts, black_col, prefs, custom_lift=prefs["LIFT_ARC"])
        draw_points(ctx, shaders, pts, black_col, 4, prefs)

    if visual_pts:
        draw_polyline(ctx, shaders, visual_pts, orange_col, prefs, custom_lift=prefs["LIFT_ARC"] + 0.5)
...
def draw_preview_1point(ctx, shaders, prefs):
    ...
    compass_col = get_axis_aligned_color(orient_normal, (0.0, 0.0, 0.0, 1.0)) if orient_normal else (0,0,0,1)
    if center and Xc and Yc:
        draw_compass_geometry(ctx, shaders, center, Xc, Yc, state["compass_rot"], prefs["COMPASS_SIZE"], state["angle_increment"], compass_col, prefs)
    ...
    if state["stage"] == 1 and state["current"] is not None:
        ...
        if state["start"]: draw_line(ctx, shaders, pv, state["start"], prefs["COL_START"], prefs)
        pts = state.get("preview_pts", [])
        if pts:
            draw_polyline(ctx, shaders, pts, (0,0,0,1), prefs)
            draw_points(ctx, shaders, pts, (0,0,0,1), pt_size, prefs)
...
def draw_preview_2point(ctx, shaders, prefs):
    ...
    if state["stage"] == 0:
        target = state.get("snap_point") if state.get("snap_point") else state.get("current")
        if target: draw_points(ctx, shaders, [target], (0,0,0,1), pt_size, prefs)
    pv = state["pivot"]
    if not pv: return
    ...
    if state["stage"] == 1 and state["current"] is not None:
        ...
        col = get_axis_aligned_color(state["current"] - pv, prefs["COL_CHORD"])
        ...
        if p1 and p2:
            col_c = get_axis_aligned_color(p2 - p1, prefs["COL_CHORD"])
            draw_line(ctx, shaders, p1, p2, col_c, prefs)
        if state["start"] is not None:
            col_h = get_axis_aligned_color(state["start"] - pv, prefs["COL_HEIGHT"])
            draw_line(ctx, shaders, pv, state["start"], col_h, prefs)
        pts = state.get("preview_pts", [])
        if pts:
            draw_polyline(ctx, shaders, pts, (0,0,0,1), prefs)
            draw_points(ctx, shaders, pts, (0,0,0,1), pt_size, prefs)
...
def draw_preview_3point(ctx, shaders, prefs):
    ...
    if state["stage"] == 1 and state["current"] is not None:
        ...
        if p1: draw_points(ctx, shaders, [p1], (0,0,0,1), pt_size, prefs)
        if p2: draw_points(ctx, shaders, [p2], (0,0,0,1), pt_size, prefs)
        if p1 and p2: draw_line(ctx, shaders, p1, p2, (0.5, 0.5, 0.5, 0.5), prefs)
        if p2 and p3: draw_line(ctx, shaders, p2, p3, (0.5, 0.5, 0.5, 0.5), prefs)
        pts = state.get("preview_pts", [])
        if pts:
            draw_polyline(ctx, shaders, pts, (0,0,0,1), prefs)
            draw_points(ctx, shaders, pts, (0,0,0,1), pt_size, prefs)
...
def draw_preview_circle_3point(ctx, shaders, prefs):
    ...
    if state["stage"] == 1 and state["current"] is not None:
        ...
        if p1 and p2: draw_line(ctx, shaders, p1, p2, prefs["COL_CHORD"], prefs)
        if p2 and p3: draw_line(ctx, shaders, p2, p3, (0.5, 0.5, 0.5, 0.5), prefs)
        pts = state.get("preview_pts", [])
        if pts:
            draw_polyline(ctx, shaders, pts, (0,0,0,1), prefs)
...
def draw_preview_ellipse(ctx, shaders, prefs):
    ...
    if mode == "ELLIPSE_CORNERS" and state["stage"] == 1 and state["current"] is not None and state["Xp"]:
    ...
    if not center: center = state.get("last_surface_hit")
    if center and state["Xp"]:
    ...
    if mode == "ELLIPSE_FOCI":
        if "f1" in state and state["f1"]: draw_points(ctx, shaders, [state["f1"]], (0,0,0,1), pt_size, prefs)
        if state["stage"] >= 1:
            f2 = state.get("f2") if state.get("f2") else state.get("current")
            if f2:
                draw_points(ctx, shaders, [f2], (0,0,0,1), pt_size, prefs)
                if state["f1"]: draw_line(ctx, shaders, state["f1"], f2, prefs["COL_CHORD"], prefs)

    if mode == "ELLIPSE_CORNERS" and state["stage"] == 1:
        cur = state["current"]
        if cur and state["Xp"]:
            d_vec = cur - pv
            w = d_vec.dot(state["Xp"]); h = d_vec.dot(state["Yp"])
            p1 = pv
            p2 = pv + state["Xp"] * w
    ...
    if state["stage"] == 1 and state["current"] is not None:
        ...
        if mode == "ELLIPSE_ENDPOINTS" and "p1" in state and "p2" in state:
            if state["p1"] and state["p2"]: draw_line(ctx, shaders, state["p1"], state["p2"], prefs["COL_START"], prefs)
...
def draw_cb_3d():
    ...
    try:
        ...
        if mode == "POINT_BY_ARCS":
            ...
            if center and Xc and Yc:
                draw_compass_geometry(ctx, shaders, center, Xc, Yc, state["compass_rot"], settings["COMPASS_SIZE"], state["angle_increment"], (0,0,0,1), settings)
        ...
        elif mode == "LINE_POLY" or mode == "CURVE_INTERPOLATE":
            if state.get("stage", 0) == 0:
                target = state.get("snap_point") if state.get("snap_point") else state.get("current")
                if target: draw_points(ctx, shaders, [target], (1.0, 0.5, 0.0, 1.0), 6, settings)
            if state.get("pivot"):
                draw_points(ctx, shaders, [state["pivot"]], (0,0,0,1), 4, settings)
            
            pts = state.get("preview_pts", [])
            if pts and len(pts) > 1:
                # Draw all segments except the last one in black
                if len(pts) > 2:
                    draw_polyline(ctx, shaders, pts[:-1], (0,0,0,1), settings)
                
                # Draw the LAST segment with axis color logic
                p_prev = pts[-2]
                p_curr = pts[-1]
                diff = p_curr - p_prev
                segment_color = get_axis_aligned_color(diff, (0,0,0,1))
                draw_line(ctx, shaders, p_prev, p_curr, segment_color, settings)
                
                if mode == "LINE_POLY":
                    draw_points(ctx, shaders, pts, (0,0,0,1), 4, settings)
        ...
        elif mode in ["POLYGON_CENTER_CORNER", "POLYGON_CENTER_TANGENT", "POLYGON_CORNER_CORNER"]:
