...
class SpatialGrid:
    ...
    def _cells_for_segment(self, p0, p1):
        """Return every grid cell touched by a segment using half-cell sampling."""
        delta = p1 - p0
        seg_len = delta.length
        if seg_len <= 1e-8:
            return {self._cell_key(p0)}
        ...
        for i in range(steps + 1):
            t = i / steps
            result.add(self._cell_key(p0.lerp(p1, t)))
    ...
    def _incremental_add(self, obj, bm, mw):
        ...
        for i in range(self._binned_edges, ne):
            ...
            self._add_to_cell(center_key, "edge_centers", center)

            for ck in self._cells_for_segment(v1w, v2w):
...
def _collect_search_cells(ctx, obj, region, rv3d, x, y, ray_origin, ray_dir, max_px):
    """Hybrid coarse search: ray cells plus a small cursor-depth volume.

    The ray keeps the search narrow in depth, while the depth volume catches
    projected-near elements that sit just outside the ray's world-space column.
    """
    is_ortho = rv3d.view_perspective == 'ORTHO'
    anchor_points = []

    # Prefer the actual mesh depth under the cursor when available.
    hit_loc, _, hit_obj = raycast_under_mouse(ctx, x, y)
    if hit_loc is not None and hit_obj == obj:
        anchor_points.append(hit_loc)

    if is_ortho:
        cursor_world = view3d_utils.region_2d_to_location_3d(
            region, rv3d, (x, y), rv3d.view_location
        )
        anchor_points.append(cursor_world)

        # Ortho rays are parallel — make a fake camera so marching still spans depth.
        fake_cam = Vector(rv3d.view_location) - ray_dir * rv3d.view_distance
        angled_dir = (cursor_world - fake_cam).normalized()
        ray_args = (fake_cam, angled_dir)
        ray_kwargs = {"max_depth": rv3d.view_distance + ctx.space_data.clip_end}
    else:
        # If nothing was hit, sample around a reasonable working depth instead of
        # relying on the infinitely thin mouse ray alone.
        if not anchor_points:
            fallback_depth = max(rv3d.view_distance, _spatial_grid.cell_size)
            anchor_points.append(ray_origin + ray_dir * fallback_depth)
        ray_args = (ray_origin, ray_dir)
        ray_kwargs = {}

    search_radii = []
    search_padding = 1
    for anchor in anchor_points:
        radius = max(_estimate_search_radius(region, rv3d, anchor, max_px), _spatial_grid.cell_size)
        search_radii.append((anchor, radius))
        search_padding = max(search_padding, int(math.ceil(radius / _spatial_grid.cell_size)))

    nearby_cells = set(
        _spatial_grid.get_cells_along_ray(*ray_args, padding=search_padding, **ray_kwargs)
    )

    for anchor, radius in search_radii:
        offset = Vector((radius, radius, radius))
        nearby_cells.update(
            _spatial_grid.get_cells_in_bounds(anchor - offset, anchor + offset, padding=1)
        )

    result = list(nearby_cells)
    _spatial_grid.debug_searched_cells = result
    return result


def _edge_screen_hit(mouse, p1_2d, p2_2d):
    """Return screen-space distance/factor for the closest point on a segment."""
    ...
    factor = max(0.0, min(1.0, (pt_on_seg_2d - p1_2d).length / seg_len))
    dist2 = (mouse - pt_on_seg_2d).length_squared
    return dist2, factor, pt_on_seg_2d
...
def snap_to_mesh_components(ctx, obj, x, y, max_px=ELEMENT_SNAP_RADIUS_PX,
    ...
    # Collect a coarse set of potentially relevant cells before doing the
    # exact screen-space snap checks.
    ...
    is_ortho = rv3d.view_perspective == 'ORTHO'
    nearby_cells = _collect_search_cells(ctx, obj, region, rv3d, x, y, ray_origin, ray_dir, max_px)
