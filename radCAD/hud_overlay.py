...
def draw_hotkeys_panel():
    ...
    if tool_mode == "2POINT":
        # 2-Point Phrasing
        lock_text = "L: Axis Locked" if is_locked else "L: Lock Axis"
    else:
        # Standard Phrasing (1-Point/3-Point)
        lock_text = "L: Plane Locked" if is_locked else "L: Lock Plane"
    ...
    if tool_mode == "LINE_POLY":
        lines.append(("Shift: Lock Axis", None))
    ...
    try:
        ...
        if state["show_measure"]:
            ...
            if state["input_mode"] == 'RADIUS': 
                label = "D:" if tool_mode == "2POINT" else "R:"
            ...
            if state["stage"] == 2:
                # --- HIDE ANGLE IF 2POINT ---
                if tool_mode != "2POINT" and tool_mode != "CIRCLE_TAN_TAN_TAN":
