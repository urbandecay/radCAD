...
class RADCAD_Preferences(bpy.types.AddonPreferences):
    ...
    preview_vertex_size: bpy.props.IntProperty(
        ...
        description="Controls the size of the little black dots shown during drawing",
        default=4,
        min=1,
        max=20
    )
    ...
    def draw(self, context):
        ...
        if col_global:
            ...
            # --- Preview Vertex Size ---
            split_vsize = col_global.split(factor=0.5, align=True)
            row_label = split_vsize.row()
            row_label.separator()
            row_label.label(text="Preview Vertex Size:", icon='BLANK1')
            split_vsize.prop(self, "preview_vertex_size", text="")
